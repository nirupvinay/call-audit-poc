...
For numeric values, a sequence may be reconstructed when digits are clearly split by spaces, punctuation, or timestamp-like formatting artifacts and the surrounding exchange supports a single continuous number. Example: if the direct answer to a PIN-code question is transcribed as '732 02:01', '732 0 201', or '732-201', it may be treated as the continuous six-digit sequence '732201' when the surrounding exchange shows the speaker is answering a PIN-code question and no competing numeric interpretation exists
A damaged numeric sequence may be accepted only when the agent repeats the same sequence and the customer confirms it, or when the same complete number appears clearly elsewhere in the transcript.
...
Permitted (confirmation repair): if a customer’s answer is garbled or partially corrupted and the agent immediately restates a single clear interpretation (for example, 'aapke naam par aata hai?') and the customer continues without contradiction or gives an affirmative continuation, the clarified version may be used for evaluation.
